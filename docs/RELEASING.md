...
2. Run the local acceptance gate on that exact tree: `./scripts/run-quality-gate.sh`, hook tests, `actionlint`, and `git diff --check`.
...
```bash
for type in container npm maven nuget rubygems; do
