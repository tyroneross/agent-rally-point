...
1. **Provision before launch.** In `command_run` (before `backend_runner.start()`), create a linked worktree for the agent on a per-agent branch off the run base (default the current branch or `main`). Reuse a small helper; set `ManagedSession.cwd` to the worktree path so the backend (tmux/cmux/ptyd) launches the agent there. Fail-closed: if worktree creation fails, surface a clear error rather than silently launching in the shared checkout.
