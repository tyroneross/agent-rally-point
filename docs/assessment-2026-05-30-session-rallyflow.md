...
| **HIGH** | **Repo-scope write guard + external-intake quarantine** — every mutating command resolves `repo_root`, `repo_id`, and `ledger_scope`; external project assessments are rejected or routed to a neutral intake surface instead of being promoted into this repo's backlog/ledger. | **B18** |
...
| MED | **Stale-claim/liveness projection** — derive stale claims from last-seen seq, heartbeat, and claim age; surface release prompts and stale warnings at read time. No daemon, no auto-release: rally records and projects; the lead/user decides. | B12/B11 |
...
9. **Repo scope is a truth boundary, not just a path prefix.** — *Why:* a different repo's assessment briefly entered the agent-rally-point coordination surface because it looked useful to this repo. That created ledger/backlog contamination: the content may be relevant as external research, but it is not agent-rally-point state. — *How:* every artifact/handoff/assessment carries `repo_root`, `repo_id`, and `ledger_scope`; external material is routed to its owning repo or a neutral intake surface, and only distilled repo-local actions enter this backlog.
...
5. **Substrate-trust backlog:** B10/B11 (id + path enforcement), B12 (reader projection + picked_up trace + stale-claim/liveness projection), B13 (session-bound receipt), B18 (repo-scope guard), the parallel-launch race.
...
8. **Repo-scope guard** so external repo assessments never re-enter this repo's backlog or ledger as first-class state.
