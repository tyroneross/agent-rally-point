...
Two symptoms surfaced in the fleet: (a) Codex sessions spamming `hook exited with code 127`, and (b) `rally sessions` listing dead "zombie" sessions. Independent Claude and Codex investigations reached the **same root causes** — and the same conclusion that these are **two separate workstreams**: this doc owns the Rally Point one (session liveness + command behaviour); host/global hook hygiene is split out to its own artifact (§6). This note records the converged evidence, the fix already applied, and — for the `rally stop` bug — a fix written to serve the *long-term* Rally objective, not just silence the symptom.
...
**F2 — The zombie-session bug is current code, in the `rally stop` ordering.** `crates/rally-cli/src/lib.rs:3690-3706`:
```rust
SessionAction::Stop => {
    let commands = backend_runner.stop_commands(&live_target);
    if !dry_run {
        backend_runner.stop(&live_target)?;          // 3693 — `?` short-circuits if the pane is already gone
        // worktree cleanup is best-effort (3702: `let _ = run_worktree::cleanup(...)`)
        remove_session_record(&session.session_id)?; // 3704 — appends the "stopped" fact (→ lib.rs:3744)
    }
    (commands, None)
}
```
If the tmux pane is already gone, `backend_runner.stop` returns `Err`, the `?` returns early, and the **"stopped" fact is never appended** (`remove_session_record` → `session_fact(…, "stopped")`) — so the session stays "active" in every projection (`rally sessions`, `find_session`). Exactly the seq-1873 incident. *(Line numbers above are the `main` checkout as of 2026-06-06 and drift across worktrees — anchor on the symbols `SessionAction::Stop` / `remove_session_record` / `find_session`. find_session is currently `lib.rs:4131` on main.)*
...
These are **contended `lib.rs` / repo-source edits in a lead-coordinated room** (`whoami`: lead `claude_code:l4`, mission "single owner does reconciliation / no fleet deploy without owner go," active claims on `lib.rs`, a `codex-injection-reliability` worktree on the hook domain). Therefore:

- **Do not fork.** Route Tier 1 + Tier 2 + the CI/PR46 port through the lead; claim `lib.rs` (or the session-lifecycle module) before editing; one owner per path.
- Suggested sequencing: F1 (done) → Tier 1 (tiny, unblocks seq-1873) → Tier 2 (liveness projection) → CI to `check-ci --strict` (`rally-gate.yml:17`) → port only the missing PR46 pieces (`next.stale_bases[]`, `room.receipts[]` field) if wanted. No push to origin without owner go (mission).
...
- [ ] Fail-open test for the **in-repo** `rally-coordination-hook.sh` under `env -i PATH=/usr/bin:/bin` → exit 0, no deny/block — runs in **repo CI** (`cargo test`). *(Workstream A — repo)*
- [ ] `cargo test -p rally-cli` green, including the new liveness + hook tests.
- [ ] CI runs `check-ci --strict`. *(F4)*
...
| F2 | `SessionAction::Stop` (`lib.rs:3690-3706`); `remove_session_record` (`lib.rs:3736`, appends `:3744`) | `?` on `backend_runner.stop` short-circuits tombstone on dead pane |
| F2 | `find_session` (`lib.rs:4131`) | projection has no liveness check |
