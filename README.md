...
source events. `judge` returns the current allow/stop decision for a phase.
`hook before-write --auto-claim` can claim a path when safe or stop on handoffs,
blockers, stale checkpoints, or claim conflicts. `rally ci gate` gives CI a
merge-time coordination check.
