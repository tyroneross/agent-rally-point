...
// ===========================================================================
// Phase 1b — per-agent linked worktree provisioning (SPEC-rally-run-worktree-isolation)
// ===========================================================================
...
/// This is the load-bearing test for the spec's anti-dormancy rule:
/// the feature must be active out of the box, not opt-in.
#[test]
fn rally_run_default_provisions_per_agent_worktree() {
    ...
    // No --shared, no --no-worktree, no RALLY_NO_WORKTREE in env.  The
    // default path MUST provision a worktree.
    let run = workspace.json(&[
        "run",
        "claude",
        "--json",
    ...
    // Branch must be `rally/<session-id>` (distinct from main).
    assert_eq!(branch, format!("rally/{session_id}"));
    assert_ne!(branch, "main", "agent branch must NOT be main");

    // Worktree dir exists on disk under .rally/worktrees/<session-id>/.
    ...
    let expected_wt = workspace
        ...
        .join("worktrees")
        .join(session_id);
    // Canonicalize both sides — on macOS the workspace temp dir is
    // resolved to `/private/tmp/...` by `git rev-parse`, while the
    // bare `PathBuf::from(temp_dir())` returns `/tmp/...`.
    assert_eq!(
    ...
    // The agent's cwd is the worktree, not the canonical checkout.
    assert_eq!(cwd, worktree_path);

    // The worktree's HEAD points at the per-agent branch.
    ...
    // The launch command's working dir is the worktree (the agent will land there).
...
/// Two agents launched in the same repo get TWO distinct linked
/// worktrees on TWO distinct branches, but BOTH resolve to the SAME
/// `.rally/` coordination room (via git common-dir).  This is the
/// structural property the spec calls "load-bearing".
#[test]
fn two_rally_runs_get_distinct_worktrees_sharing_one_room() {
    ...
    // The canonical .rally/ room lives in the original checkout, NOT in
    // either linked worktree — and both worktrees' rally invocations
    // resolve to it via the git common-dir mechanism.  Smoke-check that
    // the room has been used (facts.db exists in the canonical checkout
    // but NOT in the linked worktrees).
    ...
    assert!(
        ...
        "linked worktree A must NOT carry its own facts.db (shared via common-dir)"
    );
    assert!(
        !PathBuf::from(wt_b).join(".rally").join("facts.db").exists(),
        "linked worktree B must NOT carry its own facts.db (shared via common-dir)"
    );

    // Sanity: a `rally room` query from inside the linked worktree
    // resolves to the same room and sees the two active sessions.
    ...
    assert_eq!(
        sessions.len(),
        2,
        "rally sessions called from inside one linked worktree must see BOTH agents (shared room)"
    );

    workspace.cleanup();
}

/// `rally stop` removes the per-agent worktree (and its branch, when
/// safe).  Verified by `git worktree list` returning no orphan entry.
#[test]
fn rally_stop_removes_per_agent_worktree() {
    ...
    // Stop the session.  The backend stop uses /usr/bin/true so it
    // succeeds even though no real tmux session is running.
    ...
    // `git worktree list` must NOT mention the agent worktree any more.
    ...
    // Empty (no-commits-since-base) branch is also cleaned up.
...
/// `rally run --shared` opt-out keeps today's behavior: no worktree,
/// agent launches in the canonical checkout.
#[test]
fn rally_run_shared_flag_opts_out_of_worktree() {
    let _run_guard = serialize_rally_run();
    // Use the stub-`.git` workspace; we want to prove --shared makes
    // worktree provisioning a no-op even when `git worktree add` would
    // fail.  Critically: do NOT set the env-var escape; only the flag
    // exempts.
    let mut workspace = Workspace::new("rally-run-shared-flag");
    workspace.suppress_worktree = false; // env-var escape OFF — flag is the only opt-out
    ...
    assert!(
        ...
        "--shared must skip worktree provisioning (worktree_path null/absent)"
    );
    assert!(
        run["data"]["run"]["session"]["branch"].is_null(),
        "--shared must skip branch creation (branch null/absent)"
    );
    ...
    assert_eq!(
        ...
        "--shared session's cwd must be the canonical checkout, not a worktree"
    );

    // `--no-worktree` is the documented synonym; verify it behaves identically.
...
/// `rally run --dry-run` (default isolated) must NOT touch the filesystem
/// but MUST report the planned worktree path + branch in the envelope.
#[test]
fn rally_run_dry_run_reports_planned_worktree_without_touching_disk() {
    let _run_guard = serialize_rally_run();
    // Stub workspace is fine — dry-run doesn't invoke `git worktree add`.
    ...
    // No worktree was actually created on disk.
    assert!(
