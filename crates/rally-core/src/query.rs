...
impl TraceProjection {
    ...
    // TraceProjection methods for `recent_changes`, `profile`,
    // `active_tasks`, `artifacts`, `decisions`, `lessons`, and
    // `subscription` were deleted after the graph migration —
    // all callers moved to `crate::graph::*_typed` queries. The methods
    // below are the ones diagnose.rs and the in-memory tests still need.
