...
| 4 | **authority** | `read-only \| edit \| commit \| merge \| push` — **commit + merge are delegated to frontier agents**; the lead **audits post-hoc** (source-verify + test-evidence) and reverts only on a failed audit, rather than pre-gating. Each merge posts a merge artifact with evidence. |
