...
| **B-test-flake** | `user_journey::rally_run_reserves_numbered_ids_under_parallel_launch` is non-deterministic under cargo's parallel test runner — passes 53/53 single-threaded, fails ~1-in-3 in the parallel binary. Pre-dates B19 (pre-merge main `98737ac` flakes identically); the B11-race *runtime* fix (`fcb81ca`) holds, this is *test isolation*: sibling `rally run` tests contend on shared global state (real ledger / HOME / session registry). Fix: isolate each test's ledger/HOME (temp dir per test) or `#[serial]`. | open — test-only, not a product regression; surfaced during B19 merge validation 2026-05-31 |
