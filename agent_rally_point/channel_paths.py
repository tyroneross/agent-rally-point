...
# Sibling-module imports — clean now that we live in a standalone package
# (in build-loop, channel_paths.py used an importlib hack to load _paths.py
# via explicit file path because of a basename collision risk; here in
# agent-rally-point we have no such risk and can use normal imports).
#
# Try relative import first (package-installed mode); fall back to absolute
# (cd-into-module direct-test mode).
try:
    from ._paths import SUBCOMPONENT_PATTERNS, _safe_project_tag, derive_slug_from_cwd
except ImportError:
    from _paths import SUBCOMPONENT_PATTERNS, _safe_project_tag, derive_slug_from_cwd  # type: ignore[no-redef]
